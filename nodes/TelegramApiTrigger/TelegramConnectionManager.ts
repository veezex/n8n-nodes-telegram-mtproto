  ...
  async getClient(credentials: TelegramCredentials): Promise<TelegramClient> {
    ...
    try {
      await client.start({
        ...
        password: async () => {
          throw new Error(
            'Two-factor authentication detected. Please disable 2FA temporarily or provide password functionality.'
          )
        },
        phoneCode: async () => {
          throw new Error(
            'Phone verification code is required. Please ensure you have authorized this application first.'
          )
        },
        onError: (err) => {
      ...
      if (error instanceof Error) {
        throw new Error(
          `Authentication failed: ${error.message}. Please ensure your credentials are correct and you have authorized this application with Telegram.`
        )
      }
      throw error
    }
